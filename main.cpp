...
//  quite fast: 4 and, 14 shifts, 3 or, 3 assignment.
void PCF8574::reverse() {
	uint8_t x = _dataOut;
	x = (((x & 0xAA) >> 1) | ((x & 0x55) << 1));
	x = (((x & 0xCC) >> 2) | ((x & 0x33) << 2));
	x = ((x >> 4) | (x << 4));
...
int discover_line(int from, int to) {
	...
	for (int i = ROW_ADDRESS_START; i <= ROW_ADDRESS_END; i++) {
...
void discover_row_col_devices() {
	...
	const int COLUMN_ADDRESS_END = COLUMN_ADDRESS_START + ADDRESS_COUNT_LIMIT;

	ROW_ADDRESS_COUNT = discover_line(ROW_ADDRESS_START, ROW_ADDRESS_END); 
	COLUMN_ADDRESS_COUNT = discover_line(COLUMN_ADDRESS_START, COLUMN_ADDRESS_END); 
...
int read_buttons() {
	...
	const int COLUMN_ADDRESS_END = COLUMN_ADDRESS_START + COLUMN_ADDRESS_COUNT;

	int count = 0; 


	for (int row_address = ROW_ADDRESS_START; row_address < ROW_ADDRESS_END; row_address++) {
		for (int column_address = COLUMN_ADDRESS_START; column_address < COLUMN_ADDRESS_END; column_address++) {
